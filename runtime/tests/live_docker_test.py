...
print(f'Status: {s}, Runtime Identity: {d.get("runtime_identity")}, Epistemic: {d.get("epistemic", {}).get("status")}')
...
assert s == 200 and 'observe.navigation' in caps and 'observe.authorized_content' in caps
...
# Command execution directive rejection
...
s, d = post_json('/v1/observations', {
    'id': str(uuid.uuid4()), 'schema_version': '1.0.0', 'created_at': datetime.datetime.now(datetime.timezone.utc).isoformat(),
    'source_id': 'thorium.browser', 'payload': {'url': 'https://test.com', 'password': 'secret_password_123'}
}, {'X-Cognitia-Provider-Id': 'thorium.browser', 'X-Cognitia-Capability': 'observe.navigation'})
print(f'Credential Leak Rejected: Status {s}, Type: {d.get("error_type")}')
assert s == 422

# Oversized payload rejection
s, d = post_json('/v1/observations', {
    'id': str(uuid.uuid4()), 'schema_version': '1.0.0', 'created_at': datetime.datetime.now(datetime.timezone.utc).isoformat(),
    'source_id': 'thorium.browser', 'payload': {'huge': 'X' * (550 * 1024)}
}, {'X-Cognitia-Provider-Id': 'thorium.browser', 'X-Cognitia-Capability': 'observe.navigation'})
print(f'Oversized Payload Rejected: Status {s}, Type: {d.get("error_type")}')
assert s == 413
